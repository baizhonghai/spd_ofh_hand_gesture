...
function option = set_Option(name,type_F,type_C)
    ...
    if strcmp(type_C,'RGB')
        ...
        option.data_Path = './NorthWestern_Mat'; 
    ...
    switch name
        ...
        case 'CG'
            ...
            option.root_Path = './Cambridge_Hand_Gesture';
            option.mat_Path = [option.data_Path,'/mat_',option.name_Dataset];
            option.com_Path = [option.data_Path,'/com_',option.name_Dataset];                                
            option.res_Path = [option.data_Path,'/res_',option.name_Dataset];   
            option.num_Ite = 100;
            ...
            option.type_Norm_RLDV = 's'; 
        case 'NG'
            option.name_Dataset = 'NG';
            option.root_Path = '/Users/bzh/Code/doctor/RiemannianCovDs/HandGestureRecognition/datasets/Northwestern_Hand_Gesture_IMG';
            %option.root_Path = '/Users/baizhonghai/TP/HandGestureRecognition/datasets/Northwestern_Hand_Gesture_IMG';
            %没用% option.root_Path = '/Users/baizhonghai/TP/HandGestureRecognition/datasets/Northwestern_Hand_Gesture_IMG_keyframe';
            option.mat_Path = [option.data_Path,'/mat_',option.name_Dataset];
            option.com_Path = [option.data_Path,'/com_',option.name_Dataset];                                
            option.res_Path = [option.data_Path,'/res_',option.name_Dataset];   
            option.num_Ite = 100;
            option.num_Class = 10;
            option.num_Sample = 105;
            option.num_Gallery = 20;
            option.num_Probe = option.num_Sample - option.num_Gallery;
            option.label_Gallery = reshape(ones(option.num_Gallery,option.num_Class)*diag([1:option.num_Class]),[1,option.num_Class*option.num_Gallery]);
            option.label_Probe = reshape(ones(option.num_Probe,option.num_Class)*diag([1:option.num_Class]),[1,option.num_Class*option.num_Probe]);
            option.type_Image = '.jpg';                                                                                       
            option.resized_Row = 240;% todo 原始480
            option.resized_Col = 320;% todo 原始640，这里可以scale down一下，反正就识别那么一个小动作，那么高清其实没啥意义。
            option.block_Row = 60;
            option.block_Col = 80;
            option.block_Depth = 1;
            option.step_Temporal = 1; 
