...
function accuracy = nn_Via_disMatrix(option,ind_Matrix,disMatrix)
    ...
    label_Gallery = label_Matrix_Col(1,ind_Gallery);%由于聚类的已知标签
    label_Probe = label_Matrix_Col(1,ind_Probe);%ind_Probe 需要检测的类别，label_Probe指准确的标签
    [~,ind_Label] = min(disMatrix_Gallery2Probe);%距离最近的index
