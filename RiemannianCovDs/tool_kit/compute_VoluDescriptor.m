...
%每个图拆成81个小block,每个block的size是32*32， （先进行的整个图的特征提取），每个小图的特征数是32*32*11，这个是
%配置的参数里算出来的，可以理解成每个像素用11个特征值去描述了，那么就是1024*11个特征。
%求向量的均值和方差，就是简单的求平均值就是1024个record在11个特征维度上的均值，协方差矩阵也是一样求的。
