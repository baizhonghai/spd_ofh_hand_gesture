...
addpath('functions')
%maindir = './datasets/Cambridge_Hand_Gesture';
%savedir = './datasets/Cambridge_Hand_Gesture_keyframe';
maindir = './datasets/Northwestern_Hand_Gesture_IMG';
savedir = './datasets/Northwestern_Hand_Gesture_IMG_keyframe2';
subdir =  dir( maindir );
X=[];
for i = 1 : length( subdir )
	...
	for j = 1 : length( subsubdirpath )
        if( isequal( subsubdirpath( j ).name, '.' ) || ...
            isequal( subsubdirpath( j ).name, '..' ) || isequal( subsubdirpath( j ).name, '.DS_Store' ))
            continue;
        end

            sequenc = fullfile( maindir, subdir( i ).name, subsubdirpath( j ).name);
            sequenc = '/Users/baizhonghai/TP/HandGestureRecognition/datasets/Northwestern_Hand_Gesture_IMG/03/03_Thumb_01';%to delete it
            ...
            for k=1:frame_num-2
                ...
                image_path = fullfile( sequenc, subsubsubdirpath(k+2).name);
                I=imread(image_path);
                image_entropy=entropy(I);% important
                X(k)=image_entropy;
            end
            ...
            key_frames_labels = cluster_dp(point_data); %important
            ...
            for k=1:length(key_frames_labels)
                ...
                key_image_filename = fullfile(maindir, subdir( i ).name, subsubdirpath(j).name, sprintf('frame-%04d.jpg', key_frames_labels(k)-1)); %Action3D
