...
savedir = './datasets/Northwestern_Hand_Gesture_IMG_keyframe';
subdir =  dir( maindir );
X=[];
	...
	for j = 1 : length( subsubdirpath )
        if( isequal( subsubdirpath( j ).name, '.' ) || ...
            ...
            for k=1:length(key_frames_labels)
                ...
                key_image_filename = fullfile(maindir, subdir( i ).name, subsubdirpath(j).name, sprintf('frame_%04d.jpg', key_frames_labels(k))); %Action3D
